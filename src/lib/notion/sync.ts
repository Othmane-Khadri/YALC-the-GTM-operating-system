...
export async function runSync(opts: SyncOptions): Promise<void> {
  ...
  if (direction === 'push' || direction === 'both') {
    ...
    await pushCampaignMetricsToNotion(config)
    await pushVariantStatsToNotion(config)
  }
...
export async function syncVariantStatsToNotion(config: GTMOSConfig): Promise<void> {
  const allVariants = await db.select().from(campaignVariants)
...
async function pushCampaignMetricsToNotion(config: GTMOSConfig): Promise<void> {
  await syncCampaignMetricsToNotion(config)
  console.log('[sync] Pushed campaign metrics to Notion')
}

async function pushVariantStatsToNotion(config: GTMOSConfig): Promise<void> {
